"""
Training script for Chronic Kidney Disease (CKD) Prediction model.
...
from src.preprocessing.kidney_preprocessing import clean_ckd_dataframe, KIDNEY_FEATURE_ORDER
RAW_PATH = os.path.join(BASE_DIR, "datasets", "raw", "kidney.csv")
CLEAN_PATH = os.path.join(BASE_DIR, "datasets", "processed", "kidney_clean.csv")
...
def train_kidney_model():
    print("=" * 60)
    print(" TRAINING CHRONIC KIDNEY DISEASE (CKD) MODEL ")
    print("=" * 60)

    if not os.path.exists(RAW_PATH):
        raise FileNotFoundError(f"Raw kidney dataset not found at {RAW_PATH}")

    df_raw = pd.read_csv(RAW_PATH)
    print(f"Loaded raw dataset: {df_raw.shape[0]} rows, {df_raw.shape[1]} columns")

    df_clean = clean_ckd_dataframe(df_raw)
    df_clean.to_csv(CLEAN_PATH, index=False)
    print(f"Cleaned dataset saved to {CLEAN_PATH} ({df_clean.shape[0]} rows, {df_clean.shape[1]} columns)")

    X = df_clean[KIDNEY_FEATURE_ORDER]
    y = df_clean["target"]
    ...
    metadata = {
        ...
        "feature_names": KIDNEY_FEATURE_ORDER,
